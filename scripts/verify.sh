...
grep -q 'CREATE TABLE v2_tasks' "$TMP_DIR/alembic.sql"
grep -q 'CREATE TABLE v2_task_ns' "$TMP_DIR/alembic.sql"
grep -q 'ALTER TABLE scene_overlay_events ADD COLUMN client_time' "$TMP_DIR/alembic.sql"
