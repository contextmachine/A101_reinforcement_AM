...
V2_OPERATIONS = {
    ...
    "v2_create_verification": (V2_TAG_INDEX, "v2: запустить проверку армирования", "Если переданы bars (стержни решения), проверяются именно они — без раскладки и дозаполнения; иначе сначала выполняется раскладка стержней по переданным zones (как в POST /v2/bars), затем для каждого полигона считается требуемое и фактическое армирование. Фактическое — по геометрии самих стержней без учёта зон и их шага: стержень с диаметром d работает в полосе до середины расстояния до соседнего стержня, но не дальше 5·(cover_mm + d/2) от оси; результат не зависит от разбиения на конечные элементы. Возвращает task_id и state=pending."),
