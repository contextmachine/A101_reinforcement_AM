...
1. Создайте reusable-сцену через `POST /v1/scenes/dxf_upload`, `json_upload`, `tables_upload` или `pkl_upload`. API синхронно разбирает источник, строит raw/smooth и стабильные компоненты; успешный ответ содержит `scene_id` и `state=ready`.
...
### Компоненты и N
Компоненты определяются один раз при подготовке scene и имеют одинаковые стабильные индексы для raw/smooth и всех overlays. Overlay не пересобирает компоненты, даже если `removed` геометрически разделил их.
В `PUT /v1/tasks` поле components принимает либо список явных индексов `[0,2,...]`, либо ровно один специальный селектор: `[-2]` — все компоненты + whole, `[-3]` — все компоненты без whole, `[-1]` — только whole. Смешивать специальные значения с индексами нельзя.
N одного solver-запуска для одной компоненты или whole ограничен **100**. Перед solver worker вычисляет `max_useful_n` отдельным MILP покрытия слоёв и обрезает только запрошенные пользователем N. Для каждой единицы порядок идёт от краёв к центру (например 1,7,2,6,3,5,4), а между компонентами используется round-robin. Итоговый N комбинированного решения может быть больше 100, потому что это сумма уже рассчитанных компонент и отдельный solver на эту сумму не запускается.

### Решения, масса и compact_zones
Координаты, диаметр, шаг и длина — **мм**; масса — **кг**. `proxy_mass` — целевая оценка оптимизатора, **не физические кг**. По умолчанию `anchor_factor=40`. При вычислении objective кандидаты считаются имеющими анкеровку с двух концов даже у края поля; candidate допустим только если его расчётный прямоугольник целиком лежит в физически существующей области.
...
OPERATIONS = {
    "create_scene_dxf_upload": (0, "Создать сцену из DXF", "Загружает DXF, синхронно в API формирует raw/smooth и стабильные компоненты и создаёт reusable scene_id без запуска анализа. Успешный ответ сразу возвращает state=ready."),
    "create_scene_json_upload": (0, "Создать сцену из JSON", "Загружает JSON source-polygons и создаёт scene_id без task_id. Raw, smooth и стабильные компоненты подготавливаются синхронно в API; успешный ответ сразу ready."),
    "create_scene_tables_upload": (0, "Создать сцену из трёх XLSX", "Загружает nodes/elements/loads XLSX и синхронно в API формирует source-polygons, raw/smooth и стабильные компоненты. Успешный ответ сразу содержит ready scene_id."),
    ...
    "get_scene": (1, "Получить состояние сцены", "Возвращает состояние scene_id и сформированные при загрузке стабильные компоненты. Новые upload-ручки возвращают scene уже ready; preparing сохраняется только для совместимости со старыми/восстанавливаемыми данными."),
    ...
    'start_analysis': (3, 'Запустить анализ готовой сцены', 'Тело: scene_id, smooth, overlay_id, components, n и config. config — объект параметров либо JSON-строка как в upload. n — явный список снаружи config. [-2] все+whole; [-3] все без whole; [-1] только whole; либо список неотрицательных индексов. Отрицательный overlay сразу фиксируется как фактический положительный id. Scene ещё не готова — 409. Возвращает task_id и scene_id. Для безопасного повтора запроса клиенту следует избегать повторного создания task без контроля ответа.'),
    ...
    'list_components': (4, 'Получить компоненты анализа', 'Стабильные component_id определены по исходной физической геометрии scene, одинаковы для raw/smooth и всех overlays. Возвращаются подготовленные выбранные компоненты task и их max_useful_n; до подготовки список может быть пуст. Чтение не запускает solver. -1 обозначает whole.'),
    ...
    'schedule_component_n': (4, 'Запустить выбранные N одного компонента', 'Тело {n:[1,2,3]}. Нужна завершённая подготовка и max-N MILP, иначе 409. Для нового task допустимы только компоненты, выбранные при его создании. -1 — whole, если он выбран. N ограничен max_useful_n и 100. Ответ о постановке не означает завершение расчёта.'),
    ...
    'list_results': (5, 'Получить результаты в формате старого фронта', 'Словарь сводок с N в качестве ключа. Лучшее решение выбирается по допустимости, затем минимальной actual_mass_kg; is_optimal используется только при равной массе. Сравниваются whole и комбинации компонент. Для новых task контекст зафиксирован, для legacy без параметров raw/0. Это не рейтинг по массе unclipped.'),
