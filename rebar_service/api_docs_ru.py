...
В `PUT /v1/tasks` поле components принимает либо список явных индексов `[0,2,...]`, либо ровно один специальный селектор: `[-2]` — все реальные компоненты + виртуальный агрегат, `[-3]` — только реальные компоненты, `[-1]` — только агрегат. Явные индексы валидируются после task-декомпозиции. При одной реальной компоненте `-1` является alias на component 0 и не создаёт повторных jobs.
N одного solver-запуска для реальной компоненты ограничен **100**. Перед solver worker вычисляет её `max_useful_n`; aggregate `max_useful_n` равен сумме максимумов реальных компонент. Для aggregate total T планируются только локальные n_i, способные участвовать в `sum(n_i)=T`; отдельного whole-solver нет. Локальный `N=infeasible` является нормальным результатом только этой пары component+N: остальные N и компоненты продолжают считаться, а агрегатор использует другие feasible комбинации. `max_useful_n=0` у нового анализа означает только отсутствие реальных компонент.
...
OPERATIONS = {
    ...
    'start_analysis': (3, 'Запустить анализ готовой сцены', 'Тело: scene_id, smooth, overlay_id, components, n и config. config — объект параметров либо JSON-строка как в upload. n — явный список снаружи config. API синхронно определяет фон и task-компоненты до постановки solver jobs. [-2] все реальные+aggregate; [-3] только реальные; [-1] aggregate; либо список неотрицательных task-component индексов. Отрицательный overlay сразу фиксируется как фактический положительный id. Scene ещё не готова — 409. Возвращает task_id и scene_id. Для безопасного повтора запроса клиенту следует избегать повторного создания task без контроля ответа.'),
    ...
    'list_components': (4, 'Получить компоненты анализа', 'Возвращает только реальные task-scoped компоненты, полученные после определения фонового армирования для зафиксированных variant+overlay+config. При одной компоненте список содержит только id=0; отдельной строки -1 нет. -1 существует только как виртуальный aggregate/alias на отдельной ручке. Чтение не запускает solver.'),
    "get_component": (4, "Получить описание компонента", "Возвращает геометрические границы, polygon_indices, классы, состояния и max_useful_n. Для нового task component_id=-1 — виртуальный агрегат реальных компонент: max_useful_n является их суммой; при одной компоненте это alias на id=0 без повторного расчёта. Если реальных компонент нет, aggregate имеет max_useful_n=0. Legacy-task сохраняет старую whole-семантику." + _CTX),
    'schedule_component_n': (4, 'Запустить выбранные N одного компонента', 'Тело {n:[1,2,3]}. Нужна завершённая подготовка max-N реальных компонент, иначе 409. Для нового task -1 планирует virtual aggregate: total N раскладывается на допустимые локальные n_i и ставятся только solve_component jobs. При одной компоненте -1 маршрутизируется на 0. Infeasible одного локального N не блокирует другие N. Ответ о постановке не означает завершение расчёта.'),
    ...
    "add_n": (3, "Запустить или дополнить расчёт N", "Тело: {\"n\":5} или {\"n\":[1,2,5]}. Для нового scene-task компоненты уже определены синхронно при создании task; запрос дополняет requested N и планирует необходимые solve_component jobs. Для aggregate total N вычисляются допустимые локальные диапазоны n_i. Legacy-task может по-прежнему лениво поставить старую подготовку. GET /components сам solver не запускает. Возвращается план, а не готовый результат." + _CTX),
...
PARAMETERS = {
    ...
    "component_id": "ID реальной task-компоненты; для нового task -1 — виртуальный агрегат (при K=1 alias на component 0), для legacy-task — исторический whole.",
    ...
    "whole": "Перекрывает config.whole: для нового task включает виртуальный aggregate результатов реальных компонент; отдельного whole-solver нет.",
...
MODEL_FIELDS = {
    ...
    "whole": "Для нового task публиковать виртуальный aggregate реальных компонент; legacy-task сохраняет отдельный whole.",
