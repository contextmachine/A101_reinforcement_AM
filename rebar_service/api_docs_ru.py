...
V2_MODEL_FIELDS = {
    ...
    "fill_gaps": "Дозаполнение пропусков (по умолчанию true): после раскладки каждый полигон проверяется моделью верификации, и над каждым полигоном с недобором в самый широкий промежуток между параллельными стержнями вставляется один стержень; повторяется до полного покрытия. Вставленные стержни возвращаются как дополнительные однострежневые зоны.",
