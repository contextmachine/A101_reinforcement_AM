...
1. Создайте reusable-сцену через `POST /v1/scenes/dxf_upload`, `json_upload`, `tables_upload` или `pkl_upload`. Ответ содержит `scene_id` и `state=preparing`; тяжёлая материализация выполняется worker-ом.
2. Дождитесь `state=ready` через `GET /v1/scenes/{scene_id}`. На этой стадии уже зафиксированы исходные source-polygons, raw/smooth и стабильные индексы компонент.
...
OPERATIONS = {
    "create_scene_dxf_upload": (0, "Создать сцену из DXF", "Загружает DXF и создаёт reusable scene_id без запуска анализа. Тяжёлая материализация выполняется worker-ом; первоначальное состояние preparing."),
    "create_scene_json_upload": (0, "Создать сцену из JSON", "Загружает JSON source-polygons и создаёт scene_id без task_id. Raw, smooth и стабильные компоненты подготавливаются асинхронно."),
    "create_scene_tables_upload": (0, "Создать сцену из трёх XLSX", "Загружает nodes/elements/loads XLSX, создаёт scene_id и асинхронно формирует source-polygons, raw/smooth и стабильные компоненты."),
    ...
    "get_scene": (1, "Получить состояние сцены", "Возвращает состояние scene_id и сформированные при загрузке стабильные компоненты. Пока state=preparing запуск анализа возвращает 409."),
