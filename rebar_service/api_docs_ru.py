...
V2_OPERATIONS = {
    ...
    "v2_create_bars": (V2_TAG_INDEX, "v2: разложить стержни по зонам", "Принимает scene_id, smooth, overlay_id, config и zones (ровно одна зона kind=bg). Ставит задачу в изолированную очередь раскладки и возвращает task_id и state=pending. При fill_gaps=true (по умолчанию) после раскладки выполняется дозаполнение пропусков по модели верификации."),
    "v2_get_bars": (V2_TAG_INDEX, "v2: получить результат раскладки", "Возвращает state, а при success — bars, zones и mass_metrics."),
    "v2_create_verification": (V2_TAG_INDEX, "v2: запустить проверку армирования", "Если переданы bars (стержни решения), проверяются именно они — без раскладки и дозаполнения; иначе сначала выполняется раскладка стержней по переданным zones без дозаполнения пропусков (проверяется то, что дают сами зоны), затем для каждого полигона считается требуемое и фактическое армирование. Фактическое — по геометрии самих стержней без учёта зон и их шага: стержень с диаметром d работает в полосе до середины расстояния до соседнего стержня, но не дальше 5·(cover_mm + d/2) от оси; результат не зависит от разбиения на конечные элементы. Возвращает task_id и state=pending."),
...
V2_MODEL_FIELDS = {
    ...
    "min_bar_gap_mm": "Не используется раскладкой v2 (стержни ставятся точно по позициям зон и сдвигаются только правилом зазора); параметр сохранён для совместимости.",
    ...
    "fact_load_sm2/m": "Фактическое армирование полигона, см²/м: средняя по полигону «размазанная» плотность стержней. Каждый стержень работает в полосе до середины расстояния до соседнего параллельного стержня, но не дальше 5·(cover_mm + d/2) от своей оси (EN 1992-1-1 7.3.4); плотность точки = 10·π(d/2)²/ширина полосы ближайшего стержня.",
    "bars": "Стержни решения (Bar[]) для проверки как есть; если переданы, zones не раскладываются. Нужны zones или bars.",
    "cover_mm": "Защитный слой бетона до грани стержня, мм (по умолчанию 30). Задаёт боковую зону влияния стержня 5·(cover_mm + d/2) при проверке.",
    "smoothing_mm": "Окно усреднения размазанной плотности поперёк стержней, мм (по умолчанию 300 — шаг фонового армирования): убирает ±2–3 % зависимости результата от того, где границы элемента попадают в рисунок стержней; реальные пропуски шире окна остаются видны. 0 — без усреднения.",
    "fill_gaps": "Дозаполнение пропусков (по умолчанию true): после раскладки каждый полигон проверяется моделью верификации, и над каждым полигоном с недобором в самый широкий промежуток между параллельными стержнями вставляется один стержень; повторяется до полного покрытия. Вставленный стержень — сдвиг шага внутри зоны, в которую он вставлен: он получает zone_id этой зоны, список зон не меняется.",
    "repair": "Отчёт дозаполнения пропусков: passes, rods_added, added_kg, short_before/short_after (полигоны с недобором и худший недобор, см²/м).",
    "reason": "Почему задача не может быть посчитана (ключ): reinforcement_capacity — набор арматуры не даёт требуемого армирования; candidate_cover — поле не покрывается прямоугольниками. Текст для человека — в error.",
    "details": "Факты к reason: для reinforcement_capacity — load (требуется, см²/м), max_supported_load (максимум набора), shortfall, max_layers, back_grid, elements/elements_total/area_m2 (затронутые элементы), elements_by_load.",
    "max_useful_n": "Верхняя граница полезного N для сцены: при большем N решение не меняется, такие N возвращаются со status=infeasable и пояснением в error. Известна после подготовки задачи.",
    "min_useful_n": "Нижняя граница полезного N: меньшим числом зон требование не покрывается.",
    "solver": "Сведения о решателе для этого N (альтернативный решатель: pool, lattice, candidates, status, t_solve, t_total); отсутствует у штатного конвейера.",
